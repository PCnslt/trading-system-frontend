...
try:
    ...
    stocks = scanner.get_all_stocks(use_cache=True, limit=5)
...
try:
    ...
    if "error" not in result:
        print(f"   ✅ Technical analysis for AAPL: {result.get('final_signal')}")
        print(f"      Confidence: {result.get('confidence', 0):.1%}")
    else:
        print(f"   ⚠️  Technical analysis simulation: {result.get('error')}")
except Exception as e:
...
try:
    ...
    print(f"   ✅ Signal aggregation: {aggregated.get('total_analyzed', 0)} stocks")
    
    if aggregated.get("buy_recommendations"):
        final_decision = leader.make_final_decision(aggregated)
        print(f"   ✅ Final decision: {final_decision.get('decision', 'N/A')}")
    else:
        print("   ⚠️  No BUY recommendations in test data")
