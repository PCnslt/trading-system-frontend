#!/usr/bin/env python3
"""
Test all trading system features: UI buttons, API endpoints, progress bars, monitoring
"""
...
def test_all_features():
    ...
    if success:
        print(f"   ✅ Trading signals: {len(result.get('signals', []))} signals")
        tests.append(("Trading Signals", True))
    else:
        print(f"   ❌ Trading signals failed: {result}")
    ...
    if success:
        ...
        print(f"   ✅ Technical analysis: {signal} ({confidence:.1%} confidence)")
        tests.append(("Technical Analysis", True))
    else:
        print(f"   ❌ Technical analysis failed: {result}")
    ...
    if success:
        signal = result.get('signal', 'Unknown')
        confidence = result.get('confidence', 0)
        print(f"   ✅ Fundamental analysis: {signal} ({confidence:.1%} confidence)")
        tests.append(("Fundamental Analysis", True))
    else:
        print(f"   ❌ Fundamental analysis failed: {result}")
    ...
    if success:
        ...
        print(f"   ✅ Agent 1 status: {status}")
        tests.append(("Agent Control", True))
    else:
        print(f"   ⚠️  Agent control: {result} (may need activation)")
    ...
    if success:
        ...
        print(f"   ❌ Market data failed: {result}")
    ...
    if success:
        ...
        print(f"   ✅ Consensus analysis: {len(consensus)} symbols analyzed")
        tests.append(("Consensus Analysis", True))
    else:
        print(f"   ❌ Consensus failed: {result}")
    ...
    try:
        ...
        if frontend_response.status_code == 200:
            print("   ✅ Frontend is running at http://localhost:4201")
        ...
        print("   ❌ Frontend not accessible at http://localhost:4201")
